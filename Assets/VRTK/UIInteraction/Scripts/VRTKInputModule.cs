...
namespace UnityEngine.EventSystems
{
    ...
    /// </summary>
    public class VRTKInputModule : PointerInputModule
    {
        [Tooltip("Object which points with Z axis. E.g. CentreEyeAnchor from OVRCameraRig")]
        public Transform rayTransform;

        public VRTKCursor m_Cursor;

        public BooleanAction buttonClick;
        public Vector2Action axis2DAction;

        [Tooltip("Keyboard button to act as gaze click")]
        public KeyCode gazeClickKey = KeyCode.Space;

        [Header("Physics")] [Tooltip("Perform an sphere cast to determine correct depth for gaze pointer")]
        public bool performSphereCastForGazepointer;

        [Header("Gamepad Stick Scroll")] [Tooltip("Enable scrolling with the right stick on a gamepad")]
        ...
        [Header("Touchpad Swipe Scroll")] [Tooltip("Enable scrolling by swiping the touchpad")]
        ...
        [Tooltip("Invert X axis on touchpad")] public bool InvertSwipeXAxis = false;


        // The raycaster that gets to do pointer interaction (e.g. with a mouse), gaze interaction always works
        [NonSerialized] public VRTKRaycaster activeGraphicRaycaster;

        [Header("Dragging")] [Tooltip("Minimum pointer movement in degrees to start dragging")]
        public float angleDragThreshold = 1;

        [SerializeField] private float m_SpherecastRadius = 1.0f;
        ...
        [Header("Standalone Input Module")] [SerializeField]
        ...
        [SerializeField] private bool m_AllowActivationOnMobileDevice;
        ...
        protected void CopyFromTo(VRTKPointerEventData @from, VRTKPointerEventData @to)
        {
        ...
        protected Dictionary<int, VRTKPointerEventData>
            m_VRRayPointerData = new Dictionary<int, VRTKPointerEventData>();


        protected bool GetPointerData(int id, out VRTKPointerEventData data, bool create)
        {
            if (!m_VRRayPointerData.TryGetValue(id, out data) && create)
            {
                data = new VRTKPointerEventData(eventSystem)
                {
                    pointerId = id,
                };
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            VRTKRaycaster vrtkRaycaster = raycast.module as VRTKRaycaster;
            ...
            if (vrtkRaycaster)
            {
                ...
                leftData.position = vrtkRaycaster.GetScreenPosition(raycast);
            ...
            VRTKPointerEventData rightData;
            GetPointerData(kMouseRightId, out rightData, true);
            CopyFromTo(leftData, rightData);
            rightData.button = PointerEventData.InputButton.Right;

            VRTKPointerEventData middleData;
            ...
            m_MouseState.SetButtonState(PointerEventData.InputButton.Right, PointerEventData.FramePressState.NotChanged,
                rightData);
        ...
        protected MouseState GetCanvasPointerData()
        {
            ...
            if (activeGraphicRaycaster)
            {
                ...
                VRTKRaycaster vrtkRaycaster = raycast.module as VRTKRaycaster;
                if (vrtkRaycaster) // raycast may not actually contain a result
                {
                    // The Unity UI system expects event data to have a screen position
                    // so even though this raycast came from a world space ray we must get a screen
                    // space position for the camera attached to this raycaster for compatability
                    Vector2 position = vrtkRaycaster.GetScreenPosition(raycast);
        ...
        protected bool _previousButtonValue;
        ...
        {
            var currentButtonValue = buttonClick.Value;
            var pressed = (!_previousButtonValue && currentButtonValue) || Input.GetKeyDown(gazeClickKey);
            var released = (!currentButtonValue && _previousButtonValue) || Input.GetKeyUp(gazeClickKey);
            _previousButtonValue = currentButtonValue;
        ...
        protected Vector2 GetExtraScrollDelta()
        {
            ...
            {
                Vector2 s = axis2DAction.Value; // OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick);
                s.x *= -1; //I don't know, but it's needed
